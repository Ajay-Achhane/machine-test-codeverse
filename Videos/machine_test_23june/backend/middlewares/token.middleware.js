const jwt = require("jsonwebtoken");
const SECRET_KEY = "your-secret-key";
const constants = require("../constants/en");

const verifyToken = (allowedRoles) => (req, res, next) => {
  const token = req.headers.authorization?.split(" ")[1];
  if (!token) {
    return res
      .status(constants.UNAUTHORIZED_STATUS_CODE || 401)
      .json({ success: false, message: constants.ACCESS_DENIED_ERROR || "Access denied" });
  }

  try {
    const decoded = jwt.verify(token, SECRET_KEY);
    if (!allowedRoles.includes(decoded.role)) {
      return res.status(constants.FORBIDDEN_STATUS_CODE || 403).json({
        success: false,
        message: `Access denied. Required role: ${allowedRoles.join(" or ")}`,
      });
    }
    req.user = decoded;
    next();
  } catch (err) {
    return res
      .status(constants.UNAUTHORIZED_STATUS_CODE || 401)
      .json({ success: false, message: constants.TOKEN_INVALID || "Invalid token" });
  }
};

module.exports = {
  verifyUserToken: verifyToken(["user", "admin"]),
  verifySuperAdminToken: verifyToken(["super_admin"]),
  verifyMasterToken: verifyToken(["master"]),
  verifyTenantUserToken: verifyToken(["tenant_user", "master"]),
};
